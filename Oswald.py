#This is a modified version of pender's code here:
#https://github.com/pender/chatbot-rnn

#I've replaced the chatbot command-line interface function 
#with multiple variations of it, in order for different parts of 
#the facebook bot to pass text i/o through.
#I've also added a copy of main() as main2(), 
#and modified the end of the script to run chatbot functions 
...
#duplicate main for messager thread 
def main2():
    assert sys.version_info >= (3, 3), \
    "Must be run in Python 3.3 or later. You are running {}".format(sys.version)
    parser = argparse.ArgumentParser()
...
#Redesigned chatbot functions below. Replaces the cli interface in Pender's code with 
#web scraping functions that allow the Chatbot to interact with Facebook


#initiate instances of the Selenium Facebook "bot" class from fbbot.py
bot=fbbot()
bot.set_page_load_timeout(20)
bot.login("yourfacebookemail@example.com","yourfacebookpassword")


#Listens to Facebook notifications for interactions.  Checks latest notification every 10 seconds, with a 20 second 
#page load timeout, and compares it to the last link it interacted with.  If they are not the same link, it
#checks the text of the notificaiton.  If it contains "replied" (a new comment in a thread it's engaged in), 
#commentReplyCommenter navigates to the url in the notification, and gets the text from the last comment-reply in the thread.  
#This text is then passed as input from bot.msg to the chatbot, and commentInPost() posts it's response to the thread.
#
...
class EchoBot(Client):
    def onMessage(self, author_id, message_object, thread_id, thread_type, **kwargs):
        ...
        if author_id != self.uid:
            ...
            self.msgo = message_object
            self.stopListening()
            self.tID = thread_id
            self.tTYP = thread_type


client = EchoBot("yourfacebookemail@example.com","yourfacebookpassword")

#Modified function for moving text between fbchat and the seq2seq chatbot
def chatbot2(net, sess, chars, vocab, max_length, beam_width, relevance, temperature, topn):
    non_bmp_map = dict.fromkeys(range(0x10000, sys.maxunicode + 1), 0xfffd)
    states = initial_state_with_relevance_masking(net, sess, relevance)
    while 1:
        client.listen()
...
#Threads up the messager bot seperately from the commenter/tagger and runs both simultaneously
