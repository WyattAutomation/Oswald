...
#Redesigned chatbot functions that connect to facebook
...
bot2=fbbot()
bot2.set_page_load_timeout(20)
bot2.login("yourfacebookemail@example.com","yourfacebookpassword")
...
def chatbot(net, sess, chars, vocab, max_length, beam_width, relevance, temperature, topn):
    ...
    while True:
        ...
        #IMPORTANT: if you get an error where it hangs right after the logins and models load, 
        #try changing "by" in the line just below these comments to "ca".  This element is different across profiles, 
        #and I'm working on a handler for this!!
...
def chatbot2(net, sess, chars, vocab, max_length, beam_width, relevance, temperature, topn):
    ...
    while True:
        ...
        for i in ids:
            t = '//*[@id="u_0_' + str(i) + '"]'
            b = bot2.find_element_by_xpath(t)
            j = b.find_element_by_link_text("Full Story")
            url2 = j.get_attribute('href')
            print(url2)
            bot2.get(url2)
            q = bot2.find_element_by_xpath('//*[@id="u_0_0"]')
            try:
                m = bot2.find_element_by_css_selector("p")
                print(m.text.translate(non_bmp_map))
                user_input = m.text.translate(non_bmp_map)
                user_command_entered, reset, states, relevance, temperature, topn, beam_width = process_user_command(
                    user_input, states, relevance, temperature, topn, beam_width)
                if reset: states = initial_state_with_relevance_masking(net, sess, relevance)
                if not user_command_entered:
